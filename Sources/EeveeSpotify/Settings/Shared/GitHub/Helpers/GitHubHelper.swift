...
struct GitHubHelper {
    ...
    func getLatestRelease() async throws -> GitHubRelease {
        let data = try await perform("/repos/whoeevee/Beam/releases/latest")
    ...
    func getContributors() async throws -> [GitHubUser] {
        let data = try await perform("/repos/whoeevee/Beam/contributors")
        return try decoder.decode([GitHubUser].self, from: data)
    }
    ...
    func getEeveeContributorSections() async throws -> [EeveeContributorSection] {
        let (data, _) = try await URLSession.shared.data(
            from: URL(
                string: "https://raw.githubusercontent.com/whoeevee/Beam/swift/contributors.json"
            )!
        )
